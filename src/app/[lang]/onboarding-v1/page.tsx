...
function OnboardingPageContent({ params }: LocalizedPageProps) {
    ...
    try {
      ...
      if (hasResume && uploadedFile) {
        ...
        if (!resumeResponse.ok || !resumeResult.success) {
          toast({
            ...
            description: resumeResult.error || 'Failed to upload resume',
            variant: 'destructive'
          })
      ...
      const profileUpdate: any = {
        ...
        jobPreferences: {
          ...
          preferredSalaryMin: null as number | null,
          preferredSalaryMax: null as number | null,
          preferredCompanySizes: [],
          preferredIndustries: selectedFields,
          remoteWorkPreference: 'no_preference' as const
        },
      ...
      // Only include additional contact info if there are actual values to update
      const additionalContact: any = {}
      if (!user?.email && personalInfo.email) {
        additionalContact.email = personalInfo.email
      }
      if (!user?.phone && personalInfo.phone) {
        additionalContact.phone = personalInfo.phone
      }
      
      if (Object.keys(additionalContact).length > 0) {
        profileUpdate.additionalContactInfo = additionalContact
      }
      ...
      if (response.ok && result.success) {
        ...
        const successMessage = resumeUploadSuccess && hasResume && uploadedFile 
          ? 'Your profile and resume have been saved successfully'
  ...
  const isPersonalInfoValid = () => {
    const hasRequiredBasicInfo = personalInfo.firstName && 
                                 personalInfo.lastName && 
                                 personalInfo.dateOfBirth && 
                                 personalInfo.nationality && 
                                 personalInfo.education

    // Check that they have either provided the missing contact method OR both contacts are available
    const hasValidContacts = (user?.email && (personalInfo.phone || user?.phone)) || 
                            (user?.phone && (personalInfo.email || user?.email)) ||
                            (personalInfo.email && personalInfo.phone)

    return hasRequiredBasicInfo && hasValidContacts
  }
      ...
      <div className="container max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 flex-1 flex flex-col justify-center">
        ...
        <div className="flex justify-between items-center mt-6">
          ...
          <div className="relative group">
            ...
            <Button
              onClick={nextStep}
              disabled={isLoading || (currentStep === 1 && (hasResume ? !uploadedFile : !isPersonalInfoValid()))}
            ...
            >
              {isLoading ? (
                <>
                  ...
                  {currentStep === 2 ? 'Completing...' : 'Processing...'}
                </>
              ) : (
                <>
